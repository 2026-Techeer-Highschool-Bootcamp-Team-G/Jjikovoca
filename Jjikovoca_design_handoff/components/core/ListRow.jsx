...
export function ListRow({ icon, title, subtitle, value, valueColor, onClick, showArrow = true, divider = false }) {
  return (
    ...
    >
      {icon && (
        <span style={{ width: 40, height: 40, flexShrink: 0, borderRadius: 20, background: 'var(--color-brand-weak)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-brand)' }}>
          {icon}
        </span>
      )}
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
        <span style={{ fontSize: 17, color: 'var(--color-text-primary)' }}>{title}</span>
        {subtitle && <span style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>{subtitle}</span>}
      </div>
      <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0 }}>
        {value && <span style={{ fontSize: 15, color: valueColor || 'var(--color-text-secondary)' }}>{value}</span>}
        {showArrow && (
