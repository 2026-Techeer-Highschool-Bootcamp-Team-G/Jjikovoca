...
export function NavigationBar({ title, right, onBack }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', height: 56, padding: '0 8px', background: 'var(--color-bg-primary)' }}>
      ...
      <span style={{ flex: 1, textAlign: 'center', fontFamily: 'var(--font-sans)', fontSize: 17, fontWeight: 500, color: 'var(--color-text-primary)' }}>{title}</span>
      <div style={{ minWidth: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8 }}>{right}</div>
    </div>
  )
}
