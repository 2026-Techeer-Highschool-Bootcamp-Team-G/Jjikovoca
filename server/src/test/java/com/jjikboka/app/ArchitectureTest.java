 ...
 *
 * <p>모듈 간 허용 의존(DAG)·순환·엔티티 은닉은 Spring Modulith verify()가 담당한다(ModularityTest):
 * 역할별 하위패키지로 나뉜 모듈은 entity·repository 등이 internal이라 외부 모듈이 접근하면 verify가 막는다
 * (기존 @Entity package-private 규칙을 대체). 여기서는 Modulith가 다루지 않는 보강 규칙만 둔다 — common의 도메인 무의존.
 * allowEmptyShould(true): 대상 집합이 비어도 통과.
 */
