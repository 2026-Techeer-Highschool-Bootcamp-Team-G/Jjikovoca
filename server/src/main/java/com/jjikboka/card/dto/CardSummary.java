...
public record CardSummary(
    ...
    public static CardSummary from(Card card) {
        return from(card, GradeDistribution.ZERO);
    }

    /** 등급 카운트를 함께 실어(단어장 피드) — grade가 null이면 0으로 처리한다. */
    public static CardSummary from(Card card, GradeDistribution grade) {
        GradeDistribution g = grade == null ? GradeDistribution.ZERO : grade;
