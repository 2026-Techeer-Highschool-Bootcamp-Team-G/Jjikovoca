...
/**
 * 텍스트 스텁 렌더러 (API-25). 카드 요약을 줄글 .txt로 만든다 — 렌더 없이 흐름만 볼 때(CI·디버그)의 opt-in.
 * {@code app.export.renderer=stub}일 때만 활성(기본은 PdfBoxExportRenderer). WORD는 단어·문맥 뜻, PROBLEM은 개념·요약(정답 미노출, 13 §7).
 */
@Component
@ConditionalOnProperty(prefix = "app.export", name = "renderer", havingValue = "stub")
